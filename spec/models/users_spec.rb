...
describe User do
  ...
  describe "creating a user" do
    ...
    it 'has proposals, experiments, and comments' do
      user = User.create(email: "email@email.edu", fullname: "fullname", password: "password")
      proposal = Proposal.create(proposer_id: user.id, name: "name", hypothesis: "hypothesis")
      expect(user.proposals).to include proposal
      comment = Comment.create(commenter_id: user.id, commentable_id: proposal.id, commentable_type: "Proposal", content: "content")
